(function(){
  ...
  function buildNav(){
    ...
    screens.forEach((s, i)=>{
      ...
      li.innerHTML = `
        ...
        <span class="nav-text">${s.nav}</span>
      `;
      li.addEventListener("click", ()=>{
        if(tourMode && i>idx) return; // En modo tour, no saltar hacia adelante
        idx = i;
        activePin = 1;
        render();
      });
  ...
  function setProgress(){
    ...
    // Navegación en modo tour: marca lo alcanzado como “clickeable”
  ...
  function renderCards(screen){
    ...
    // 2) Card del pin activo (destacada arriba)
  ...
  nextBtn.addEventListener("click", ()=>{
    if(idx < screens.length-1){
      ...
      // Final: mini “cierre” sin modal pesado
      alert("Listo. Ya tienes el mapa para ubicarte: Lista/Búsqueda → Reporte → Registro → Errores/Permisos.");
    }
  });
